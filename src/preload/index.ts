...
const api = {
  ...
  dialog: {
    showCertificateTrustDialog(...options: unknown[]) {
      return ipcRenderer.invoke('dialog:showCertificateTrustDialog', ...options)
    },
    showErrorBox(...options: unknown[]) {
  ...
  getBilibiliHttpProxy() {
    return process.env.BILIBILI_HTTP_PROXY
  }
}
...
window.addEventListener('loadstart', () => {
  channelSet.forEach(channel => ipcRenderer.removeAllListeners(channel))
})
